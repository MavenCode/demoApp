...
import akka.http.scaladsl.model.{HttpResponse, HttpEntity, ContentType, MediaTypes}
...
trait SwaggerHttpService
  ...
  final def swaggerRoutes: Route =
      get {
        path(docsPath) {
          complete {
            HttpResponse(entity = HttpEntity(ContentType(MediaTypes.`application/json`), toJsonString(api.getResourceListing())))
          }
        } ~ (
          (for (
            ...
            path(docsPath / subPath.drop(1).split('/').map(
              ...
              .reduceLeft(_ / _)) {
              complete {
                HttpResponse(entity = HttpEntity(ContentType(MediaTypes.`application/json`), toJsonString(apiListing)))
              }
            }
          }).reduceLeft(_ ~ _)
        )
