...
class AkkaApiScanner(apiTypes: Seq[Type])
    extends Scanner
    with LazyLogging {
  def classes(): List[Class[_]] = {

    apiTypes.map(apiType => if (!(apiType <:< typeOf[SwaggerHttpService])) logger.warn(s"ApiType $apiType does not implement HttpService"))
